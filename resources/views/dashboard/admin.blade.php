...
<div class="pixel-modal" id="printChoiceModal" style="display:none;">
  <div class="pixel-modal-content">
    ...
    <!-- Buttons -->
    <div class="btn-row">
      <button type="button" onclick="openPrintListModal()">
        🧾 Print List Nota (Per Bulan)
      </button>
...
<!-- Modal pilih bulan -->
<div class="pixel-modal" id="printListMonthModal" style="display:none;">
  <div class="pixel-modal-content">
    <h3>Pilih Bulan</h3>
    ...
    <form method="GET" action="{{ route('nota.printListByMonth') }}" target="_blank">
      <input type="month" name="month" required>
      <div style="margin-top:10px; display:flex; gap:8px;">
        <button type="submit">Print</button>
        <button type="button" class="btn-cancel" onclick="playClickSound(); closePrintListModal()">Batal</button>
      </div>
    </form>
  </div>
</div>
...
<div class="omori-dialog">
  <div class="dialog-box">
    <div class="character-column">
      ...
      <img src="{{ asset('img/admin1.png') }}" class="dialog-portrait" />
    </div>
     </div>
    <div class="dialog-content">
      <div class="dialog-text" id="dialogText">
Selamat datang di FixQuest, Admin! Ingin tur singkat tentang fitur sistem?
      </div>
    ...
    <div class="modal" id="notaModal">
      <div class="modal-content">
       ...
       <button onclick="openFormNota()">📝 Form Nota Digital</button>
        <!-- 📤 Export by Month -->
<form method="GET" action="{{ route('notas.exportByMonth') }}">
  <div class="form-group">
  ...
  <div class="btn-row">
    <button type="submit" class="btn-save" onclick="playClickSound()">📤 Export Nota</button>

    <button type="button" class="btn-cancel" onclick="playClickSound(); closeNotaMenu()">❌ Close</button>
  </div>
</form>
...
<!-- 📤 Edit Nota Modal -->
<div class="modal" id="editNotaModal" style="display: none;">
  <div class="modal-content">
    <h3>✏️ Edit Nota</h3>
    <form id="editNotaForm" method="POST">
      ...
      <div class="form-group">
        ...
        <input type="text" name="nama_driver" id="edit_nama_driver" required>
      </div>
...
<div class="form-group kerusakan-harga-group">
  ...
  <div id="edit_items_container" style="display:flex; flex-direction:column; gap:8px;">
    <!-- rows injected by JS -->
  </div>

  <div style="margin-top:8px;">
    <button type="button" onclick="addItemRow('edit_items_container')">+ Tambah Kerusakan</button>
  </div>
</div>
      ...
      <div class="form-group">
        <label>Total (auto):</label>
        <input type="number" id="edit_total_harga" name="total_preview" value="0" readonly>
      </div>

      <div class="form-group" style="display:none;">
        <input type="hidden" id="edit_total_harga_hidden" name="harga[]">
      </div>
      ...
      <div class="btn-row">
        <button type="submit" onclick="syncTotalsBeforeSubmit('edit_items_container','edit_total_harga','edit_total_harga_hidden')">💾 Update</button>
        <button type="button" onclick="playClickSound(); closeEditNota()">❌ Batal</button>
      </div>
    </form>
  </div>
</div>
...
<div class="dashboard-board popout-shadow">
  ...
  <div class="scrollable-data">
    @forelse ($notas as $nota)
     @php
  $items = $nota->items ?: (
      ($nota->kerusakan || $nota->harga)
      ? [['kerusakan' => $nota->kerusakan, 'harga' => $nota->harga]]
      : []
  );
@endphp
...
<div class="data-box animated-card">
  ...
  <ul style="margin:6px 0 0 18px; padding:0;">
    @foreach($items as $it)
      <li>{{ $it['kerusakan'] ?? '' }} — Rp {{ number_format($it['harga'] ?? 0, 0, ',', '.') }}</li>
    @endforeach
  </ul>

  <b>Total:</b> Rp {{ number_format($nota->harga ?? array_sum(array_map(fn($i)=>$i['harga']??0,$items)), 0, ',', '.') }}<br>
...
function handleOption(option) {
  ...
  const dialogMap = {
 ya: {
  text: "Bagus! Ayo mulai hari ini dengan semangat! Silakan pilih fitur yang ingin kamu pelajari!",
  options: [
    { label: " Input Nota", value: "fitur_input" },
    { label: " Export Excel", value: "fitur_export" },
...
fitur_export: {
  text: "📤 Klik komputer, lalu pilih <strong>Export Nota</strong> untuk mulai mengekspor data nota ke Excel.\n\n - Pilih bulan yang diinginkan, lalu klik tombol Export.\n - Sistem akan otomatis mengunduh data nota dalam format Excel.",
  options: [{ label: " Kembali", value: "reset" }]
},

fitur_print: {
  text: "🖨️ <strong>Print Nota:</strong>\n\nJika ingin mencetak, klik maskot <strong>Riki</strong> di sebelah kanan layar. Setelah diklik akan muncul dua pilihan:\n\n <strong>1. Print List Nota</strong>\n - Digunakan untuk mencetak daftar semua nota dalam satu bulan.\n - Pilih bulan yang ingin dicetak, lalu klik tombol <strong>PRINT</strong>.\n - Sistem akan menampilkan preview data sebelum dicetak.\n\n <strong>2. Print Satu Nota</strong>\n - Digunakan untuk mencetak hanya satu nota tertentu.\n - Pilih id nota yang ingin dicetak, lalu klik tombol <strong>PRINT</strong>.\n\nTips: Pastikan printer sudah terhubung sebelum mencetak.",
  options: [{ label: " Kembali", value: "reset" }]
},

fitur_dashboard: {
  text: "📊 <strong>Dashboard & Pencarian:</strong>\nMenampilkan daftar seluruh nota.\n\n- Kamu bisa mencari nota berdasarkan Kode Unit, Nama Driver, Status, atau memfilter berdasarkan bulan tertentu.\n\n- Dari dashboard ini juga kamu bisa langsung mengedit data nota bila diperlukan. (Hanya Admin yang bisa mengedit Dashboard)",
  options: [{ label: " Kembali", value: "reset" }]
},
...
// --- Edit Nota flow (updated signature) ---
function editNota(id, tanggal, kode, driver, items, status) {
  // Basic fields
  document.getElementById('edit_id').value = id;
  document.getElementById('edit_tanggal_masuk').value = tanggal;
  document.getElementById('edit_kode_unit').value = kode;
  document.getElementById('edit_nama_driver').value = driver;
  document.getElementById('edit_status').value = status;

  // Build rows from items (fallback to 1 empty if none)
  const containerId = 'edit_items_container';
  const container = document.getElementById(containerId);
  container.innerHTML = '';
  const dataItems = Array.isArray(items) && items.length ? items : [{kerusakan:'', harga:''}];
  dataItems.forEach(it => addItemRow(containerId, it));
  calcTotal(containerId, 'edit_total_harga');

  // set form action
  document.getElementById('editNotaForm').action = `/notas/${id}`;
...
function openPrintList() {
  // Your existing list printer (all notas)
  window.open('{{ route('notas.print') }}', '_blank');
}
...
// 🔹 For EDIT: preload data into form
function loadEditItems(existingItems) {
  const container = document.getElementById('edit_items_container');
  container.innerHTML = ''; // clear old rows
  existingItems.forEach(item => {
    addItemRow('edit_items_container', item);
  });
}
...
// 🔹 Open "Pilih Bulan" modal for list print
function openPrintListModal() {
  closePrintMenu(); // hide the choice modal first
  const m = document.getElementById('printListMonthModal');
